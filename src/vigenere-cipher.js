class VigenereCipheringMachine {
    ...
    createString(message, key, flag) {
      ...
      key = this.keigen(message, key.toUpperCase());
      let res = "";
      for (let i = 0; i < message.length; i++) {
        if (this.abc.indexOf(message.charAt(i)) === -1) {
          ...
          if (flag === true) {
            res += this.abc.charAt((this.abc.indexOf(message.charAt(i)) + this.abc.indexOf(key.charAt(i))) % 26);
          } else {
            res += this.abc.charAt((this.abc.indexOf(message.charAt(i)) - this.abc.indexOf(key.charAt(i)) + 26) % 26);
          }
        }
      }
